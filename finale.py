...
def predict_probability(stag,gender,age,profession,traffic,coach,greywage,way,extraversion,independ,selfcontrol,anxiety,novator):
    prediction=loaded_model.predict(data_new)[:, 1]#predictions using our model
    return prediction 
def main():
    ...
    if st.button("Predict"):
        ...
        result=loaded_model.predict(data_new)[:, 1].round(3)*100   #result will be displayed if pressed
